...
class Bankday extends Data
{
    ...
    public function getEndDate($type, $start, $days, $format = null)
    {
        ...
        if ($this->getConfig($this->_configSectionId . '/' . $type . '/time_after_enable')) {
            ...
            $currentTs = $this->_objectManager->get('\Magento\Framework\Stdlib\DateTime\DateTime')->timestamp();
            list($hour, $minute) = explode(',', $this->getConfig($this->_configSectionId . '/' . $type . '/time_after'));
            if (date('H', $ts) >= $hour && date('i', $ts) > $minute && date('Y-m-d', $currentTs) == date('Y-m-d', $ts)) {
