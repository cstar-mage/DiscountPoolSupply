...
require([
    ...
    $('#container .entry-edit.form-inline').on('click', '.fieldset-wrapper .fieldset-wrapper-title', function() {
        ...
        /*Fix for category page, hide delivery types*/
        var prTimeout = setInterval(function(){

            var entrySelector = $('[name="estimated_delivery_enable"], [name="product[estimated_delivery_enable]"]');
            var date = $('[name="estimated_delivery_date_from"], [name="product[estimated_delivery_date_from]"]');

            if(entrySelector.length && date.length){
